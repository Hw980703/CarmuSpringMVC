...
public class dmstStoreLogic implements dmstStore {
	...
	public int selectListCount(SqlSession session, Map<String, String> paramMap) {
		...
		int result = session.selectOne("korFreeBoard.selectListByKewordCount", paramMap);

		return result;
	}

	@Override
	public List<dmstFreeBoard> searchNoticeByKeyword(SqlSession session, PageInfo pInfo, Map<String, String> paramMap) {
		...
		RowBounds rowbounds = new RowBounds(offset, limit);
		List<dmstFreeBoard> dList = session.selectList("korFreeBoard.selectNoticeByKeyword",paramMap,rowbounds);
